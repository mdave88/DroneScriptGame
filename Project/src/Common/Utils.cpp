	...
	*/
	...
	namespace gfx
	{
		struct ScreenText
		{
			...
			{
				this->timeToStay = EngineCore::getInstance()->getElapsedTime() + timeToStay;
			}
		};
			...
			for(size_t i = 0; i < text.length(); i++)
			{
				...
				case '<':
					if(text.substr(i + 1, 5) == "color")
					{
						...
						color = strToVec3(colorVectorStr);
		...
		*/
		void flushScreenTextBuffer()
		{
			...
			{
				return text.timeToStay < EngineCore::getInstance()->getElapsedTime();
			});
