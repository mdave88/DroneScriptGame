...
void LightSource::setVisibility(bool isVisible)
{
	...
	if (m_id < ms_enabledLights.size())
	{
		ms_enabledLights[m_id] = m_isVisible;
	}
	else
	{
		ms_enabledLights.push_back(m_isVisible);
	}
}
...
{
	return lightSourceDirectory;
}


LightSourcePtr getLightSource(const std::string& name)
{
	return lightSourceDirectory.at(name);
}
...
void LightSource::shadowingPass1(graphics::RenderContext& context)
{
	...
	graphics::getEffect("simplest")->bind();
	context.m_pShader = graphics::getEffect("simplest").get();
